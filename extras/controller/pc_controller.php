...
class pc_controller {
    ...
    public static function init() {
        ...
        $cache_id = sprintf('%X', crc32($_SERVER['QUERY_STRING'] . '-' . $_COOKIE['city_id'] . '-' . $_COOKIE['city_name'] . '-' . $_COOKIE['location_id'] . '-' . $_COOKIE['location_name']));
        ...
        if (!ecjia_front::$controller->is_cached('index.dwt', $cache_id)) {
                            ...
                            if (empty($categoryGoods[$key]['children'][$k]['goods_list'])) {
                                ...
                                if (!empty($v['cat_id'])) {
                                    foreach ($v['cat_id'] as $k1 => $v1) {
                                        $categoryGoods[$key]['children'][$k]['children'][] = array('id' => $v1['id'], 'name' => $v1['name'], 'image' => $v1['img']);
                                    }
                                } else {
            ...
            ecjia_front::$controller->assign('cat', $category);
